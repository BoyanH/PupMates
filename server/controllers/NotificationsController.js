...
exportsObj.addNotification = function (notification) {
	...
	User.findOne({_id: notification.to}, function (err, user) {
		...
		if(user && user.notifications) {
			for (var i = 0, len = user.notifications.length; i < len; i += 1) {
				...
				else if(user.notifications[i].notifType == "newMsgNotif" && user.notifications[i].from.id.toString() == notifObj.from.id) {
		...
		//If user didn't already recieve a friend request / new message alert from the same person
		if( !requestExists ) {
...
exportsObj.deleteNotification = function(req, res) {
...
exportsObj.markNotificationAsSeen = function (req, res) {
...
exportsObj.addNewFriendship = function (userOne, userTwo) {
...
exportsObj.deleteSharedNotifications = deleteSharedNotifications;